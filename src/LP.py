...
def run_lp(
    run_name: str,
    inputs_from_usr: dict | None = None,
) -> dict:
    ...
    inputs = parameters.get_base_inputs()
    # update base inputs with usr inputs
    inputs.update(inputs_from_usr or {})
    log.info("inputs:")
    pprint(inputs)
    sys.stdout.flush()
    ...
    # restrict_gas arrives as a percent of load (0-100); convert to a fraction.
    # e.g. 20 -> gas may serve at most 20% of load; 100 -> effectively no limit.
    ...
    for h in df.index:
        ...
        if h == 0:
            solver.Add(
                SOC[h] <= init_ch_level * batt_hours * batt + batt_ch[h] - batt_disch[h] / batt_eff
            )
            solver.Add(
                SOC[h] >= init_ch_level * batt_hours * batt + batt_ch[h] - batt_disch[h] / batt_eff
            )
        ...
        if use_outside_energy:
            ...
            solver.Add(
                hydro_gen[h] + solar_gen[h] + wind_gen[h] + gas_gen[h] + batt_disch[h] - batt_ch[h]
                >= df["2030_load"][h]
            )
    ...
    cap_mw = {"batt_mw": batt_mw, "solar_mw": solar_mw, "wind_mw": wind_mw, "gas_mw": gas_mw}
    ...
    # get the hourly solved values for each resource
    ...
    if use_outside_energy:
        final_df["outside_energy"] = [
            outside_energy[h].solution_value() for h in range(df.shape[0])
        ]

    # calc net load for a check on the results
    if use_outside_energy:
        final_df["net_load"] = round(
            (
                final_df["hydro"]
                + final_df["solar"]
                + final_df["wind"]
    ...
    final_df["load_and_charge"] = round((final_df["batt_charge"] + final_df["2030_load"]), 2)
    ...
    if use_outside_energy:
        ...
        log.info(
            "Outside energy as a percentage of load: {0:,.3f}%\n".format(outside_energy_percent)
        )

        total_outside_energy = final_df.outside_energy.sum()
    ...
    # total generated MWh by resource (distinct names from the earlier
    # gas_gen/wind_gen/solar_gen solver-variable lists)
    gas_gen_total = final_df.gas.sum()
    wind_gen_total = final_df.wind.sum()
    solar_gen_total = final_df.solar.sum()

    total_co2 = (
        gas_co2_ton_per_mw * cap_mw["gas_mw"]
        + gas_co2_ton_per_mwh * gas_gen_total
        + wind_co2_ton_per_mw * cap_mw["wind_mw"]
        + wind_co2_ton_per_mwh * wind_gen_total
        + solar_co2_ton_per_mw * cap_mw["solar_mw"]
        + solar_co2_ton_per_mwh * solar_gen_total
    ...
    total_cost = (
        gas_mw_cost * cap_mw["gas_mw"]
        + gas_mwh_cost * gas_gen_total
        + wind_mw_cost * cap_mw["wind_mw"]
        + wind_mwh_cost * wind_gen_total
        + solar_mw_cost * cap_mw["solar_mw"]
        + solar_mwh_cost * solar_gen_total
    ...
    log.info(
        "total time to build, solve, and verify (minutes): {0:,.2f}\n".format(
            (total_time_1 - total_time_0) / 60
        )
    )
    print("\n", flush=True)
...
# quick manual run: `python -m src.LP` (see tests/test_lp.py for assertions)
if __name__ == "__main__":
    print("\n")

    results = run_lp(run_name="test")
    log.info("cap_mw: %s", results["cap_mw"])
