...
- **file-or-directory** (optional): Path to review. If omitted, review the
  changed files: `git diff --name-only HEAD` unioned with `git ls-files
  --others --exclude-standard` — union, not fallback.
...
Say which you did. "Reused the `solved` fixture from `tests/test_lp.py`" and "the
suite had no builder for a repo in this state, so I built one" are both
acceptable; building a ninth bespoke fixture beside eight existing ones is not,
because the next reviewer then has nine to choose from and no reason to prefer
any.
...
- **Hollowed-out names** — an identifier that condensed a multi-word domain
  term until what is left names something else: `load_*` for the peak load
  input, `factor` for a resource capacity factor. The remainder
...
- **Near-identical functions** — two or more functions that share
  ...
  `capacity_boxes`, `cost_boxes` and `generation_boxes` in `app.py` share
  `metric_section(title, boxes, col_widths) -> ui.TagList` rather than
  repeating the table-rendering shell — the extraction this repo already made,
  and the shape to look for.
- **Repeated multi-line patterns in the app's renderers** — if the same
  3+ line sequence appears in several `@render` functions (build a
  figure → add a series → reorder → restyle), extract it into a helper
  in `src/utils.py`. Each renderer must stay its own decorated function
  for Shiny's reactive graph, but the *body* can call a shared helper.
...
- **When NOT to extract** — do not flag single-use patterns shorter
  than 3 lines, Shiny `@reactive` / `@render` signatures (the decorator
  and the binding name are the contract), or test setup code (test
  clarity > DRY).
...
- **The model's domain constants each need provenance.** Emission factors,
  heat rate, VOM, asset lifetimes — a bare number here is indistinguishable
  from a guess, so each needs a comment saying what it is and where it came
  from.
- **Arbitrary thresholds need the reason, not just the value.** The `0.05`
  total-import cap (`src/LP.py:262`), the `0.2` hourly-import cap
  (`src/LP.py:243`), and the small charge/discharge objective penalties are
  the local examples — the penalties exist to break ties, not to model a cost,
  and a reader who assumes otherwise will try to calibrate them.
- **`ResultsDB`'s methods catch and log.** Confirm they still *log* rather
  than silently passing; a bare `except` that swallows the error turns a
  failed save into a run that looks fine and is not there.
- **`run_lp`'s docstring lists every input key.** A change to the inputs that
  leaves it alone is an outdated-docstring finding, and it is the docstring
  most likely to be read as a contract.
- **`src/parameters.py` is the single source of truth for model inputs.** A
  default written into an `app.py` UI control that also lives in
  `parameters.py` is a source-of-truth finding — the UI then disagrees with
  the model the moment either moves. Say which copy is canonical (the model
  code wins) and whether they have already drifted.
- **`ResultsDB.table_names` in `src/db.py` is the source of truth for the
  results schema.** A parallel hardcoded list of table or column names
  anywhere else is a finding, and it is the shape that breaks a saved run
  silently rather than loudly.
- **`run_lp` is long because it is a build → solve → assemble procedure**,
  not because it is neglected. Report a specific extractable block with the
  signature it would take; do not report the length on its own.
- **An LP constraint change moves numbers no assertion pins.** Where the diff
  touches an objective term or a constraint, say which direction the result
  moves and why that is right. A changed expected value absorbed into a test
  without that explanation is a Must Fix.
...
2. Run static checks on the changed files first — these surface
   ...
   *Verify by running* above describes. **Its `commit` mode does not run the
   ...
   Treat any ruff finding as **at least Should Fix**; F821 (undefined name)
   and most B-class rules are **Must Fix** since they're real bugs. Cite the
   rule code (e.g. `F401`, `B008`) in each finding so the user knows what
   `--fix` would do. Skip the **Dead code** checklist item — ruff's
   `F401`/`F811` cover it more reliably than human review. **Do not skip
   Direct imports**: no selected rule flags a string forward reference, so
   that one stays manual. The same goes for line length and `X | None` —
   `pyproject.toml` ignores `E501` and does not select `UP`, both deliberately
   left to human review. **Do not report `S` (flake8-bandit) findings here** —
   those are security concerns owned by the `security-scan` skill (the
   `code-reviewer` agent runs it as a separate phase); reporting them here
   would double-count.

   Formatting **is** checked here — `ruff format` is enforced in this
   repo, so drift is a real finding rather than noise on files nobody
   intends to reformat.

   Note on the configured ignores: `tests/**` ignores `S101` (pytest's whole
   model is `assert`), `src/db.py` ignores `S608` and `S101`, and `scripts/**`
   ignores `S603`. Read `[tool.ruff.lint.per-file-ignores]` in
   `pyproject.toml` rather than trusting a list here; that file is the
   authority.
...
4. Apply the checklist to every changed file, checking for **correctness bugs
   ...
   drifted.
5. **Where the change touches `src/`, open `app.py`.** The app is the only
   caller of most of `src/`, and a signature or results-key change reaches it
   without appearing in the diff. Where it touches `src/db.py`, check the
   renderers that read those keys.
