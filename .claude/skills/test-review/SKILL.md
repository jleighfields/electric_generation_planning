...
- **Exclude the `e2e` marker.** It drives the Shiny app through a real
  ...
  by design.
- **A mutation against a `slow` test costs about 15 seconds**, because it runs
  a full LP solve. That is affordable for a handful and not for a sweep; say
  in the scope statement how many you ran.
- **This suite is small, so absence of coverage is the likely finding.** Ten
  tests over an LP model means most mutations will survive simply because
  nothing asserts on what they changed. Report that as a coverage gap naming
  the behavior nothing pins — it is more useful here than a list of tests that
  did redden.
- **A mutation to a constraint is the one worth constructing.** Loosen a bound
  or drop a term and see whether any test notices the objective moved. If none
  does, the model has no regression net, which is the single most valuable
  thing this pass can tell you.
