...
**Never the `e2e` marker.** It drives the Shiny app through a real browser and
needs `playwright install chromium` first; `addopts` in `pyproject.toml`
excludes it for that reason, and neither tier of this review is the place to
pay it. Where a finding turns on what the rendered UI does, it is unsettled in
...
- **The test suite once, in Phase 3 of the full pass, and it is yours rather
  than a skill's.** No skill runs the *suite*: whether a change set passes is
  a fact about the change, not a review finding, so it belongs to whatever is
  making the pass over that change — here, you. Run it as `uv run pytest -m
  "not e2e" -q`, the command `test.yml` uses. One run answers four questions
  ...
  and what baseline Phase 5 adds a failing test to. Running it before Phase 3
  and again after duplicates the first question, in every review that touches
  a comment — which is nearly all of them.
